...
'''
Author: Wei Luo
Date: 2022-04-04 17:42:30
LastEditors: Wei Luo
LastEditTime: 2022-04-05 00:13:24
Note: Note
'''
'''
...
class Ax12:
    ...
    def readData(self, id):
        ...
        try:
            ...
            if (error != 0):
                print("Error from servo: " + Ax12.dictErrors[error] +
                      ' (code  ' + hex(error) + ')')
                return -error
    ...
    def setID(self, id, newId):
        self.direction(Ax12.RPI_DIRECTION_TX)
        Ax12.port.flushInput()
        checksum = (~(id + Ax12.AX_ID_LENGTH + Ax12.AX_WRITE_DATA +
                      Ax12.AX_ID + newId)) & 0xff
        outData = chr(Ax12.AX_START)
        outData += chr(Ax12.AX_START)
        outData += chr(id)
    ...
    def setBaudRate(self, id, baudRate):
        ...
        checksum = (~(id + Ax12.AX_BD_LENGTH + Ax12.AX_WRITE_DATA +
                      Ax12.AX_BAUD_RATE + br)) & 0xff
        outData = chr(Ax12.AX_START)
        outData += chr(Ax12.AX_START)
        outData += chr(id)
    ...
    def moveSpeed(self, id, position, speed):
        ...
        checksum = (
            ~(id + Ax12.AX_GOAL_SP_LENGTH + Ax12.AX_WRITE_DATA +
    ...
    def moveSpeedRW(self, id, position, speed):
        ...
        s = [speed & 0xff, speed >> 8]
        checksum = (
            ~(id + Ax12.AX_GOAL_SP_LENGTH + Ax12.AX_REG_WRITE +
              Ax12.AX_GOAL_POSITION_L + p[0] + p[1] + s[0] + s[1])) & 0xff
        outData = chr(Ax12.AX_START)
        outData += chr(Ax12.AX_START)
        outData += chr(id)
    ...
    def setTorqueStatus(self, id, status):
        ...
        checksum = (~(id + Ax12.AX_TORQUE_LENGTH + Ax12.AX_WRITE_DATA +
                      Ax12.AX_TORQUE_STATUS + ts)) & 0xff
        outData = chr(Ax12.AX_START)
        outData += chr(Ax12.AX_START)
        outData += chr(id)
    ...
    def setLedStatus(self, id, status):
        ...
        checksum = (~(id + Ax12.AX_LED_LENGTH + Ax12.AX_WRITE_DATA +
                      Ax12.AX_LED_STATUS + ls)) & 0xff
        outData = chr(Ax12.AX_START)
        outData += chr(Ax12.AX_START)
        outData += chr(id)
    ...
    def setAngleLimit(self, id, cwLimit, ccwLimit):
        ...
        checksum = (~(id + Ax12.AX_AL_LENGTH + Ax12.AX_WRITE_DATA +
                      Ax12.AX_CW_ANGLE_LIMIT_L + cw[0] + cw[1] + ccw[0] +
                      ccw[1])) & 0xff
        outData = chr(Ax12.AX_START)
        outData += chr(Ax12.AX_START)
        outData += chr(id)
    ...
    def setPunchLimit(self, id, punch):
        ...
        checksum = (~(id + Ax12.AX_PUNCH_LENGTH + Ax12.AX_WRITE_DATA +
                      Ax12.AX_PUNCH_L + p[0] + p[1])) & 0xff
        outData = chr(Ax12.AX_START)
        outData += chr(Ax12.AX_START)
        outData += chr(id)
    ...
    def setCompliance(self, id, cwMargin, ccwMargin, cwSlope, ccwSlope):
        ...
        checksum = (~(id + Ax12.AX_COMPLIANCE_LENGTH + Ax12.AX_WRITE_DATA +
                      Ax12.AX_CW_COMPLIANCE_MARGIN + cwMargin + ccwMargin +
    ...
    def setLedAlarm(self, id, alarm):
        self.direction(Ax12.RPI_DIRECTION_TX)
        Ax12.port.flushInput()
        checksum = (~(id + Ax12.AX_LEDALARM_LENGTH + Ax12.AX_WRITE_DATA +
                      Ax12.AX_ALARM_LED + alarm)) & 0xff
        outData = chr(Ax12.AX_START)
        outData += chr(Ax12.AX_START)
        outData += chr(id)
    ...
    def setShutdownAlarm(self, id, alarm):
        self.direction(Ax12.RPI_DIRECTION_TX)
        Ax12.port.flushInput()
        checksum = (~(id + Ax12.AX_SHUTDOWNALARM_LENGTH + Ax12.AX_WRITE_DATA +
                      Ax12.AX_ALARM_SHUTDOWN + alarm)) & 0xff
        outData = chr(Ax12.AX_START)
        outData += chr(Ax12.AX_START)
        outData += chr(id)
    ...
    def readRWStatus(self, id):
        ...
        Ax12.port.flushInput()
        checksum = (
            ~(id + Ax12.AX_RWS_LENGTH + Ax12.AX_READ_DATA +
